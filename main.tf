...
resource "aws_iam_role_policy_attachment" "AmazonEKSClusterPolicy" {
  ...
  role       = aws_iam_role.eks-kanban-role-cluster.id
}
...
resource "aws_eks_cluster" "eks-kanban-cluster" {
  ...
  role_arn = aws_iam_role.eks-kanban-role-cluster.arn
  ...
  depends_on = [
    aws_iam_role.eks-kanban-role-cluster,
  ]
}
...
resource "aws_eks_node_group" "eks-kanban-workernode-group" {
  ...
  node_role_arn   = aws_iam_role.eks-kanban-role-workernodes.arn
  subnet_ids      = [ aws_subnet.teamdr-public.id, aws_subnet.teamdr-public2.id ]
  ...
  remote_access {
    ec2_ssh_key               = "Deepak's_EC2_key_pair3"
